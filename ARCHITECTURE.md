...
|-------|-------|-----------|----------|
| Hermes (planning) | `lfm2.5-1.2b-thinking-mlx` | MLX 4-bit | `127.0.0.1:1234/v1` |
| OpenClaw (coding) | `liquid/lfm2.5-1.2b` | MLX 8-bit | `127.0.0.1:1234/v1` |
...
```
Vercel (React)  →  ngrok HTTPS  →  localhost:7900 (Laravel + SQLite)
```
...
| **Cron** | `hermes-config.yaml` → `cron` entry `forge2-heartbeat` | `agent-log.md` § Autonomous run |
| **Human-in-the-loop** | Plan approval + **What Needs Your Call** gate | `agent-log.md` § Human approval gates |

## Config files (secrets removed)

- `openclaw.json` — OpenClaw @ `127.0.0.1:1234`
- `hermes-config.yaml` — Hermes @ `127.0.0.1:1234`
